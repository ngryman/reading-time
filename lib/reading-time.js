...
function readingTime(text, options) {
  ...
  let isWordBound = options.wordBound || isAnsiWordBound
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (
      isCJK(text[i]) ||
      (!isWordBound(text[i]) && (isWordBound(text[i + 1]) || isCJK(text[i + 1])))
    ) {
      words++
    }
    // In case of CJK followed by punctuations, those characters have to be eaten as well
    if (isCJK(text[i])) {
      while (i <= end && (isPunctuation(text[i + 1]) || isWordBound(text[i + 1]))) {
        i++
      }
    }
  }
